/**
 * @file bsp_driver_iwdg.c
 * @author ZB (2536566200@qq.com)
 * @brief 看门狗驱动
 * @version 0.1
 * @date 2025-09-12
 * 
 * @copyright Copyright (c) 2025
 * 
 */
#include "bsp_driver_iwdg.h"

bsp_driver_iwdg_object_t _iwdg_drv_dev[IWDG_DEV_MAX] = {0};    /**< 独立看门狗驱动内存池 */

bsp_driver_iwdg_status_e bsp_driver_iwdg_register(uint32_t index, bsp_driver_iwdg_object_t *dev)
{
    if((dev == NULL) || 
        (index >= IWDG_DEV_MAX)) {
        return IWDG_DRIVER_ERROR;
    }

    _iwdg_drv_dev[index].index = index;
    _iwdg_drv_dev[index].user_data = dev->user_data;
    _iwdg_drv_dev[index].pf_iwdg_init = dev->pf_iwdg_init;
    _iwdg_drv_dev[index].pf_iwdg_deinit = dev->pf_iwdg_deinit;
    _iwdg_drv_dev[index].pf_iwdg_feed = dev->pf_iwdg_feed;

    return IWDG_DRIVER_OK;
}

bsp_driver_iwdg_status_e bsp_driver_iwdg_get_object(uint32_t index, bsp_driver_iwdg_object_t *dev)
{
    if((index >= IWDG_DEV_MAX) || 
		(dev == NULL)){
		return IWDG_DRIVER_ERROR;	
	}

    *dev = _iwdg_drv_dev[index];

    return IWDG_DRIVER_OK;
}

bsp_driver_iwdg_status_e bsp_driver_iwdg_init(const bsp_driver_iwdg_object_t *dev)
{
    if((dev == NULL) || 
        (dev->index >= IWDG_DEV_MAX) || 
        (_iwdg_drv_dev[dev->index].pf_iwdg_init == NULL)) {
        return IWDG_DRIVER_ERROR;
    }

    _iwdg_drv_dev[dev->index].pf_iwdg_init(dev);

    return IWDG_DRIVER_OK;
}

bsp_driver_iwdg_status_e bsp_driver_iwdg_deinit(const bsp_driver_iwdg_object_t *dev)
{
    if((dev == NULL) || 
        (dev->index >= IWDG_DEV_MAX) || 
        (_iwdg_drv_dev[dev->index].pf_iwdg_deinit == NULL)) {
        return IWDG_DRIVER_ERROR;
    }

    _iwdg_drv_dev[dev->index].pf_iwdg_deinit(dev);

    return IWDG_DRIVER_OK;
}

bsp_driver_iwdg_status_e bsp_driver_iwdg_feed(const bsp_driver_iwdg_object_t *dev)
{
    if((dev == NULL) || 
        (dev->index >= IWDG_DEV_MAX) || 
        (_iwdg_drv_dev[dev->index].pf_iwdg_feed == NULL)) {
        return IWDG_DRIVER_ERROR;
    }

    _iwdg_drv_dev[dev->index].pf_iwdg_feed(dev);

    return IWDG_DRIVER_OK;
}










